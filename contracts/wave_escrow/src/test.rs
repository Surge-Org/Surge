...
impl Fixture {
    ...
    fn client(&self) -> WaveEscrowClient<'_> {
        WaveEscrowClient::new(&self.env, &self.escrow)
    }
    fn funded(&self, pool: i128) -> u64 {
        let c = self.client();
        let id = c.create(&self.manager, &200, &50, &self.dust);
        c.fund(&id, &self.sponsor, &pool);
        id
    }
    fn closed(&self, pool: i128) -> u64 {
        self.env.ledger().set_timestamp(100);
        let id = self.funded(pool);
        self.client().open(&id);
        self.env.ledger().set_timestamp(200);
...
fn lifecycle_claims_and_dust_conserve_real_token_balance() {
    ...
    for share in shares.iter() {
        ...
        assert_eq!(
            c.try_claim(&id, &share.recipient),
            Err(Ok(EscrowError::NothingToClaim))
        );
    }
...
fn dust_recipient_may_also_earn_points() {
    ...
    let shares = soroban_sdk::vec![
        &f.env,
        Share {
            recipient: f.dust.clone(),
            points: 1
        },
        Share {
            recipient: Address::generate(&f.env),
            points: 2
        }
    ];
    c.settle(&id, &shares);
    assert_eq!(c.allocation(&id, &f.dust), 2);
    assert_eq!(c.claim(&id, &f.dust), 2);
}
...
fn topups_withdrawal_and_cancellation_refund_actual_contributions() {
    ...
    token::TokenClient::new(&f.env, &f.token).transfer(&f.sponsor, &second, &300);
    c.withdraw(&id, &f.sponsor, &20);
    c.open(&id);
    c.fund(&id, &second, &300);
    c.fund(&id, &f.sponsor, &20);
    assert_eq!(c.get_wave(&id).pool, 400);
    assert_eq!(
        c.try_withdraw(&id, &second, &1),
        Err(Ok(EscrowError::WrongState))
    );
    c.cancel(&id);
    assert_eq!(c.refund(&id, &second), 300);
    assert_eq!(c.refund(&id, &f.sponsor), 100);
    assert_eq!(
        c.try_refund(&id, &second),
        Err(Ok(EscrowError::NothingToClaim))
    );
    assert_eq!(c.get_wave(&id).refunded, 400);
    assert_eq!(f.balance(&f.escrow), 0);
}
...
fn expiry_is_permissionless_and_never_settled_waves_do_not_lock_sponsors() {
    for phase in [Phase::Funded, Phase::Open, Phase::Closed] {
        ...
        assert_eq!(c.refund(&id, &f.sponsor), 50);
        assert_eq!(c.get_wave(&id).phase, Phase::Cancelled);
...
fn rejects_invalid_lifecycle_deadlines_points_and_duplicates() {
    ...
    assert_eq!(
        c.try_settle(&id, &shares),
        Err(Ok(EscrowError::DuplicateRecipient))
    );
    assert_eq!(
    ...
    assert_eq!(
        c.try_settle(&id, &f.shares(&[1; 65])),
        Err(Ok(EscrowError::TooManyRecipients))
    );
    assert_eq!(c.get_wave(&id).phase, Phase::Closed);
    assert_eq!(c.allocation(&id, &f.dust), 0);
    f.env.ledger().set_timestamp(251);
    assert_eq!(
        c.try_settle(&id, &f.shares(&[1])),
...
fn missing_trustline_has_distinct_error_and_retry_preserves_allocation() {
    ...
    let account = match other.asset() {
        xdr::Asset::CreditAlphanum4(a) => a.issuer,
        _ => panic!("expected issued asset"),
    };
    ...
    assert_eq!(
        c.try_claim(&id, &recipient),
        Err(Ok(EscrowError::MissingTrustline))
    );
    assert_eq!(c.allocation(&id, &recipient), 99);
    assert_eq!(c.get_wave(&id).claimed, 0);
    assert_eq!(f.balance(&f.escrow), 99);
    token::StellarAssetClient::new(&f.env, &f.token).trust(&recipient);
    assert_eq!(c.claim(&id, &recipient), 99);
    assert_eq!(f.balance(&recipient), 99);
}
...
fn failed_funding_rolls_back_pool_and_sponsor_entry() {
    ...
    assert_eq!(
        c.try_fund(&id, &empty, &100),
        Err(Ok(EscrowError::TransferFailed))
    );
    assert_eq!(c.get_wave(&id).pool, 1);
    assert_eq!(c.get_wave(&id).sponsors, 1);
    assert_eq!(c.contribution(&id, &empty), 0);
}
...
fn waves_remain_isolated() {
    ...
    let b = f.funded(20);
    c.cancel(&a);
    c.refund(&a, &f.sponsor);
    assert_eq!(c.get_wave(&b).pool, 20);
    assert_eq!(f.balance(&f.escrow), 20);
}
...
mod hostile {
    ...
    impl HostileToken {
        pub fn configure(env: Env, target: Address, id: u64, recipient: Address) {
            env.storage().instance().set(&HostileKey::Target, &target);
            env.storage().instance().set(&HostileKey::WaveId, &id);
            env.storage()
                .instance()
                .set(&HostileKey::Recipient, &recipient);
            env.storage().instance().set(&HostileKey::Attack, &true);
        }
        pub fn transfer(env: Env, _from: Address, _to: soroban_sdk::MuxedAddress, _amount: i128) {
            let n: u32 = env
                .storage()
                .instance()
                .get(&HostileKey::Transfers)
                .unwrap_or(0);
            env.storage()
                .instance()
                .set(&HostileKey::Transfers, &(n + 1));
            if env
                .storage()
                .instance()
                .get(&HostileKey::Attack)
                .unwrap_or(false)
            {
    ...
    fn hostile_token_cannot_reenter_and_double_claim() {
        ...
        let t = HostileTokenClient::new(&env, &token);
        let manager = Address::generate(&env);
        let recipient = Address::generate(&env);
        let id = c.create(&manager, &200, &50, &manager);
        c.fund(&id, &manager, &100);
        c.open(&id);
        ...
        t.configure(&escrow, &id, &recipient);
        let before = t.transfers();
        ...
        match c.try_claim(&id, &recipient) {
            Ok(Ok(value)) => {
                assert_eq!(value, 100);
                assert_eq!(t.transfers(), before + 1);
                assert_eq!(c.allocation(&id, &recipient), 0);
            }
            Err(Ok(EscrowError::TransferFailed)) => {
                assert_eq!(t.transfers(), before);
                assert_eq!(c.allocation(&id, &recipient), 100);
                assert_eq!(c.get_wave(&id).claimed, 0);
            }
...
fn resource_measurements_at_maximum_size() {
    for count in [1, MAX_RECIPIENTS] {
        ...
        std::println!(
            "RESOURCE native fund n={count}: {:?}",
            f.env.cost_estimate().resources()
        );
        c.open(&id);
        f.env.ledger().set_timestamp(200);
        c.close(&id);
        let points = std::vec![1;count as usize];
        let shares = f.shares(&points);
        c.settle(&id, &shares);
        std::println!(
            "RESOURCE native settle n={count}: {:?}",
            f.env.cost_estimate().resources()
        );
        c.claim(&id, &shares.get(0).unwrap().recipient);
        std::println!(
            "RESOURCE native claim n={count}: {:?}",
            f.env.cost_estimate().resources()
        );
    }
}
