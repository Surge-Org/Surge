...
fn compiled_wasm_fits_network_limits_and_claim_footprint_is_constant() {
    ...
    for count in [1, MAX_RECIPIENTS] {
        ...
        let token = env.register_stellar_asset_contract_v2(sponsor.clone()).address();
        ...
        println!("RESOURCE wasm fund n={count}: {:?}", env.cost_estimate().resources());
        c.open(&id); env.ledger().set_timestamp(200); c.close(&id);
        let shares = Vec::from_iter(&env, (0..count).map(|_| Share { recipient: Address::generate(&env), points: 1 }));
        c.settle(&id, &shares);
        println!("RESOURCE wasm settle n={count}: {:?}", env.cost_estimate().resources());
        assert_eq!(c.claim(&id, &shares.get(0).unwrap().recipient), 10_000);
        let resources = env.cost_estimate().resources();
        println!("RESOURCE wasm claim n={count}: {resources:?}");
        let footprint = (resources.disk_read_entries, resources.memory_read_entries, resources.write_entries,
            resources.disk_read_bytes, resources.write_bytes, resources.contract_events_size_bytes);
        if let Some(small) = small_claim { assert_eq!(footprint, small); } else { small_claim = Some(footprint); }
