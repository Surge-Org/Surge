...
import { initialState, isOwnApplication, pointsFor, repoName } from '../lib/model';
...
export function ContributorPoints() {
  ...
  const total = accepted.reduce((sum, a) => {
    const issue = state.issues.find(i => i.id === a.issueId);
    return sum + (issue ? pointsFor(issue.complexity) : 0);
  }, 0);

  return (
    <Page>
      <PageHead title="Points" sub="Earned when a maintainer accepts your work." />
      <div className="grid c3">
        <div className="card stat">
          <p className="label">Total points</p>
          <strong className="num">{total}</strong>
          <small>Across all waves</small>
        </div>
        <div className="card stat">
          <p className="label">Accepted</p>
          <strong className="num">{accepted.length}</strong>
          <small>Contributions</small>
        </div>
        <div className="card stat">
          <p className="label">Rewards</p>
          <strong className="num">{state.rewards.length}</strong>
          <small>Grants calculated</small>
        </div>
      </div>
